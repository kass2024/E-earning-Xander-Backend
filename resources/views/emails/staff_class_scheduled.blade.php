...
<head>
    <meta charset="UTF-8">
    <title>{{ $companyName ?? ($emailBrand['companyName'] ?? config('app.name')) }} - Class Scheduled</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    @php
        $brandName = $companyName ?? ($emailBrand['companyName'] ?? config('app.name'));
        $headerFrom = $brandHeaderFrom ?? ($emailBrand['headerFrom'] ?? '#0f172a');
        $headerTo = $brandHeaderTo ?? ($emailBrand['headerTo'] ?? '#1d4ed8');
        $primary = $brandPrimary ?? ($emailBrand['primaryColor'] ?? '#1d4ed8');
    @endphp
    <style>
        ...
        a.btn { display: inline-block; padding: 10px 18px; background-color: {{ $primary }}; color: #f9fafb !important; text-decoration: none; border-radius: 999px; font-size: 13px; font-weight: 600; }
        a.link { color: {{ $primary }}; word-break: break-all; }
    </style>
</head>
<body>
<div class="wrapper">
